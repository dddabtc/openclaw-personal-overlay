import { U as CONFIG_DIR, Ut as resolveGatewayPort, Y as ensureDir, Yt as resolveStateDir, ct as shortenHomeInString, ot as resolveUserPath, zt as resolveConfigPath } from "./entry.js";
import { i as loadConfig } from "./config-DMW11Vgn.js";
import { Jt as normalizeFingerprint, Xt as loadOrCreateDeviceIdentity, kt as PROTOCOL_VERSION, t as GatewayClient } from "./client-DrK7aLru.js";
import { h as GATEWAY_CLIENT_NAMES, m as GATEWAY_CLIENT_MODES } from "./message-channel-CVHJDItx.js";
import { r as isSecureWebSocketUrl } from "./net-DZ5Ayk-W.js";
...
function resolveGatewayCredentials(context) {
	...
	return {
		token: context.explicitAuth.token || (!context.urlOverride ? context.isRemoteMode ? trimToUndefined(context.remote?.token) : trimToUndefined(process.env.OPENCLAW_GATEWAY_TOKEN) || trimToUndefined(process.env.CLAWDBOT_GATEWAY_TOKEN) || trimToUndefined(authToken) : void 0),
		password: context.explicitAuth.password || (!context.urlOverride ? trimToUndefined(process.env.OPENCLAW_GATEWAY_PASSWORD) || trimToUndefined(process.env.CLAWDBOT_GATEWAY_PASSWORD) || (context.isRemoteMode ? trimToUndefined(context.remote?.password) : trimToUndefined(authPassword)) : void 0)
	};
}
