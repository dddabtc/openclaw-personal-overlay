...
import { Mt as loadDotEnv } from "./config-C5eGjyf_.js";
...
import { u as installUnhandledRejectionHandler } from "./runner-BGEicYhv.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-nvVa54aJ.js";
import { t as assertSupportedRuntime } from "./runtime-guard-D09-7j7u.js";
import { t as emitCliBanner } from "./banner-DujBZPrU.js";
import "./doctor-config-flow-B6Rwbo_f.js";
import { n as ensureConfigReady } from "./config-guard-BzSH8eCz.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-CaKe2UYq.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-Bd04Wmhi.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-Cueu9gE_.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-B4yi5wzT.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-B9T45K0l.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-C91DKF5f.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-C91DKF5f.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-Yi-QK-ZC.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-Yi-QK-ZC.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-BaFE4AVp.js");
	...
	if (primary) {
		...
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-JPgIfJod.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-BThBCOa4.js").then((n) => n.a);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { loadConfig } = await import("./config-C5eGjyf_.js").then((n) => n.t);
