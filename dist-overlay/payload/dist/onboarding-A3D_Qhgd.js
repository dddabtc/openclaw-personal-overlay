import { Ft as DEFAULT_GATEWAY_PORT, Ut as resolveGatewayPort, _ as defaultRuntime, ot as resolveUserPath } from "./entry.js";
...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-VhYln9Mp.js").then((n) => n.d);
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BVCxQm7_.js").then((n) => n.t);
	...
	const { applyOnboardingLocalWorkspaceConfig } = await import("./onboard-config-7cR7QGjL.js").then((n) => n.n);
	...
	const { promptCustomApiConfig } = await import("./onboard-custom-DiRsKvTz.js").then((n) => n.r);
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-D7pxrgvC.js").then((n) => n.t);
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Dz4aaA3l.js").then((n) => n.i);
	...
	const { configureGatewayForOnboarding } = await import("./onboarding.gateway-config-D3jCW5nk.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-CUze9MDn.js").then((n) => n.n);
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BnT91rj-.js").then((n) => n.t);
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-ClyPZrmC.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-BF4H14oL.js");
