...
import { f as defaultRuntime, r as enableConsoleCapture } from "./subsystem-Cf9yS0UI.js";
...
import { Kn as loadDotEnv } from "./auth-profiles-D6r73dBV.js";
...
import { i as formatUncaughtError } from "./errors-Duls987w.js";
...
import { _ as installUnhandledRejectionHandler } from "./audio-transcription-runner-BeCyq-4G.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-BQH9EaXZ.js";
import { t as assertSupportedRuntime } from "./runtime-guard-CGqgXWWP.js";
import { t as emitCliBanner } from "./banner-DIWWh4hz.js";
import "./doctor-config-flow-Dnk1mAjQ.js";
import { n as ensureConfigReady } from "./config-guard-DtCVKWV3.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-q5jfrVU_.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-BcdjX-x8.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-BChOK_bX.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-DNladnN7.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-Bl5S3v6-.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-C9cczo3c.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-C9cczo3c.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-D1AWzNV1.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-D1AWzNV1.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-DVRb2kxT.js");
	...
	if (primary) {
		const { getProgramContext } = await import("./program-context-BQ3tU1FN.js").then((n) => n.n);
		const ctx = getProgramContext(program);
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-auQTSezd.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-JZjBE3Yx.js").then((n) => n.a);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { registerPluginCliCommands } = await import("./cli-D7Mk2_re.js");
		const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-JZjBE3Yx.js").then((n) => n.a);
