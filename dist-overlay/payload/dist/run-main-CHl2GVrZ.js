...
import { Mt as loadDotEnv } from "./config-CaJqgOvf.js";
...
import "./subagent-registry-CiKr2c80.js";
...
import { u as installUnhandledRejectionHandler } from "./runner-D3O98nXv.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-Cg7HzQiI.js";
import { t as assertSupportedRuntime } from "./runtime-guard-D09-7j7u.js";
import { t as emitCliBanner } from "./banner-BGMpi3ET.js";
import "./doctor-config-flow-DnpGIeeo.js";
import { n as ensureConfigReady } from "./config-guard-GeF3_h3L.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-CiXbjtvT.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-aT10iq5U.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-BOobSz8k.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-Dc_IMmae.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-CbtYKXPl.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-D3zgaekw.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-D3zgaekw.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-DBnnxcHR.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-DBnnxcHR.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-C_HSiFmj.js");
	...
	if (primary) {
		...
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-C1gVe6-C.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-CwXfiQGI.js").then((n) => n.a);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { loadConfig } = await import("./config-CaJqgOvf.js").then((n) => n.t);
